...
`TAMER_TACTICS.md` v0.9.1 está **congelada**. Nada de este archivo se toca hasta tener datos de partidas con jugadores reales. Esto es la lista de lo que hay que medir o decidir **con** esos datos.
...
| Número | Valor v0.9.1 | Qué medir / alternativas si falla |
|---|---|---|
...
| **Ratio Tamers:Digi-Egg** | rango 4–7 : 2–5, Tamers todos distintos (v0.9.1) | Curva "grupos online por turno" según lo que elija cada jugador (7/2 vs 4/5 dan curvas muy distintas). Si un extremo del rango domina, angostar el rango. Falta calcular P(2º grupo en turno N) y P(3º grupo en turno N). Confirmar además que el piso de 4 Tamers **distintos** sea armable en mono-color con pool de Tamers chico. |
...
- **Stacking de Tamers de ramp pasivo** (varios en grupos, +memoria por turno cada uno). **Mitigado en v0.9.1** (Tamers únicos): ya no se pueden apilar copias del mismo. Queda vigilar el caso residual de 2-3 Tamers de ramp *distintos* repartidos en grupos.
